	...
	function customize_register( $wp_customize ){
		...
		if( method_exists($wp_customize, 'add_panel') ) {
			$wp_customize->add_panel( 'page_settings', array(
				'title' => __( 'Page Template Settings', 'focus' ),
				'description' => __( 'Change layouts for various pages on your site.', 'focus' ),
				'priority' => 11,
			) );
		}
		...
		$types = array();
		$templates = array(
			'home' => __( 'Blog Page', 'focus' ),
			'search' => __( 'Search Results', 'focus' ),
			'date' => __( 'Date Archives', 'focus' ),
			'author' => __( 'Author Archives', 'focus' ),
			'404' => __( 'Not Found', 'focus' ),
		);
		...
		foreach( $post_types as $post_type => $post_type_data ) {
			...
			$types[] = array(
				'group' => 'archive',
				'id' => $post_type,
				'title' => __( 'Type', 'focus' ) . ': ' . $post_type_data->label
			);
		}
		...
		foreach( $taxonomies as $tax_slug => $taxonomy ) {
			...
			$types[] = array(
				'group' => 'taxonomy',
				'id' => $tax_slug,
				'title' => __( 'Taxonomy', 'focus' ) . ': ' . $taxonomy->label
			);
		}
